...
${error.stack}`;
  ...
  var DataGroup = class {
    ...
    async getCacheNames() {
      const [cache, ageTable, lruTable] = await Promise.all([this.cache, this.ageTable, this.lruTable]);
  ...
  var AppVersion = class {
    constructor(scope2, adapter2, database, idle, debugHandler, manifest, manifestHash) {
      ...
      this.dataGroups = (manifest.dataGroups || []).map(
        (config) => new DataGroup(scope2, adapter2, config, database, debugHandler, `${config.version}:data`)
      );
...
${msgIdle}`,
  ...
  var Driver = class {
    ...
    async handleClick(notification, action) {
      ...
      const onActionClick =
        (_b = (_a = notification == null ? void 0 : notification.data) == null ? void 0 : _a.onActionClick) == null
          ? void 0
